/* MCC AUTH — one McCluster account, every McCluster-powered site.
   ============================================================
   VENDORED COPY. The canonical source is mcclusterishere/mccluster →
   js/mcc-auth.js. Do not edit this file: fix it there and copy it back, or
   the satellites drift apart from the house.

   This is not a second auth system (AGENTS.md §17 forbids one). It is the
   same Supabase project, the same auth users, the same accounts — the file
   is copied only so an artist's site does not fetch its login code from
   another origin on the critical path.
